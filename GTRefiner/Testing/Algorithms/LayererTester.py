...
if __name__ == '__main__':
    ...
    another_text_line = CommentTextLine(Polygon([(x, y + 100) for x, y in text_line_polygon.xy]),
                                     BaseLine(
                                         Line([(x, y + 100) for x, y in base_line.polygon.xy])))

    ascender_descender_line = AscenderDescenderRegion(MainTextLine(Polygon([(x-500, y + 200) for x, y in text_line_polygon.xy]),
                                     BaseLine(
                                         Line([(x-500, y + 200) for x, y in base_line.polygon.xy]))), x_height=45)
